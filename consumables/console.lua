SMODS.Consumable {
    ...
    loc_txt = {
        name = 'Console',
        text = {
            [1] = 'Add a {C:attention}Glitched Seal{} to',
    ...
    loc_vars = function(self, info_queue, card)
        local glitched_seal = G.P_SEALS and (G.P_SEALS['porkify_glitched'] or G.P_SEALS['glitched'])
        if glitched_seal then
            info_queue[#info_queue + 1] = glitched_seal
        end
        return { vars = {} }
    end,
    ...
    use = function(self, card, area, copier)
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                target:set_seal('porkify_glitched', nil, true)
                return true
            end
        }))
        G.E_MANAGER:add_event(Event({
